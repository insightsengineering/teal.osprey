    ...
    plot_r <- reactive({
      ...
      teal.code::chunks_push(bquote({
        osprey::g_spiderplot(
          ...
          vref_line = .(vref_line),
          href_line = .(href_line),
          x_label = if (is.null(formatable::var_labels(ADTR_FILTERED[.(x_var)], fill = FALSE))) {
            .(x_var)
          } else {
            formatable::var_labels(ADTR_FILTERED[.(x_var)], fill = FALSE)
          },
          y_label = if (is.null(formatable::var_labels(ADTR_FILTERED[.(y_var)], fill = FALSE))) {
            .(y_var)
          } else {
            formatable::var_labels(ADTR_FILTERED[.(y_var)], fill = FALSE)
          },
