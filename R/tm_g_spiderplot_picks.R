...
#' @examples
#' ## Example using stream (ADaM) dataset and picks method
#' data <- teal_data() %>%
#'   within({
#'     library(nestcolor)
...
#'
#' @export
tm_g_spiderplot.pick <- function(label, # nolint: object_name_linter.
                                 dataname,
                                 paramcd = teal.picks::variables(
  ...
  x_var <- teal.picks::picks(teal.picks::datasets(dataname), x_var)
  y_var <- teal.picks::picks(teal.picks::datasets(dataname), y_var)
  marker_var <- teal.picks::picks(teal.picks::datasets(dataname), marker_var)
  line_colorby_var <- teal.picks::picks(teal.picks::datasets(dataname), line_colorby_var)

  if (!is.null(xfacet_var)) {
    xfacet_var <- teal.picks::picks(teal.picks::datasets(dataname), xfacet_var)
  }
  if (!is.null(yfacet_var)) {
    yfacet_var <- teal.picks::picks(teal.picks::datasets(dataname), yfacet_var)
  }

  checkmate::assert_class(paramcd, "picks")
  if (isTRUE(attr(paramcd$variables, "multiple"))) {
    warning(
  ...
  if (isTRUE(attr(x_var$variables, "multiple"))) {
    warning(
  ...
  if (isTRUE(attr(y_var$variables, "multiple"))) {
    warning(
  ...
  if (isTRUE(attr(marker_var$variables, "multiple"))) {
    warning(
  ...
  if (isTRUE(attr(line_colorby_var$variables, "multiple"))) {
    warning(
  ...
  module(
    ...
    server = srv_g_spider.picks,
    server_args = args[names(args) %in% names(formals(srv_g_spider.picks))],
    ui = ui_g_spider.picks,
    ui_args = args,
    transformators = transformators
  )
}

ui_g_spider.picks <- function(id, ...) { # nolint: object_name_linter.
  ns <- NS(id)
  a <- list(...)
  shiny::tagList(
