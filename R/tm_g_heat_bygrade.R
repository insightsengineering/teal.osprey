...
srv_g_heatmap_bygrade <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$plot_cm, {
      ...
      ADCM_label <- variable_labels(datasets$get_data(cm_dataname, filtered = FALSE), fill = FALSE) # nolint
      variable_labels(ADCM_FILTERED) <- ADCM_label
    ...
    plt <- reactive({
      ...
      variable_labels(ADSL_FILTERED) <-
        variable_labels(datasets$get_data(sl_dataname, filtered = FALSE), fill = FALSE)
      variable_labels(ADEX_FILTERED) <-
        variable_labels(datasets$get_data(ex_dataname, filtered = FALSE), fill = FALSE)
      variable_labels(ADAE_FILTERED) <-
        variable_labels(datasets$get_data(ae_dataname, filtered = FALSE), fill = FALSE)
      ...
      if (input$plot_cm) {
        ...
        teal.code::chunks_push(bquote({
          ...
          variable_labels(conmed_data)[conmed_var] <-
            variable_labels(ADCM_FILTERED, fill = FALSE)[conmed_var]
        }))
      } else {
        teal.code::chunks_push(bquote({
