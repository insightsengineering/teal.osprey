      ...
      r = reactive({
        ...
        q1 <- if (!is.null(ae_line_col_var) && is.data.frame(ADAE)) {
          teal.code::eval_code(
            q1,
            code = substitute(
              expr = ae_line_col_name <- teal.data::formatters_var_labels(ADAE, fill = FALSE)[ae_line_col_var],
            ...
            qq <- teal.code::eval_code(
              ...
              code = substitute(
                expr = {
                  ...
                  teal.data::formatters_var_labels(ADAE)[ae_line_col_var] <- # nolint
                    teal.data::formatters_var_labels(ADAE, fill = FALSE)[ae_line_col_var]
                },
                env = list(
                  ADSL = as.name(sl_dataname),
