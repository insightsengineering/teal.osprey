    ...
    plot_r <- reactive({
      ...
      if (!is.null(input$select_ae)) {
        if (input$select_ae == FALSE | is.na(ae_dataname)) {
          ...
          teal::variable_labels(ADAE_FILTERED) <- teal::get_variable_labels(
            datasets$get_data(ae_dataname, filtered = FALSE), fill = FALSE
          )
      ...
      if (!is.null(ae_line_col_var) && is.data.frame(ADAE_FILTERED)) {
        chunks_push(bquote(ae_line_col_name <- teal::get_variable_labels(ADAE_FILTERED, fill = FALSE)[.(ae_line_col_var)]))
      } else {
      ...
      if (select_plot["ae"]) {
        ...
        if (ADSL$USUBJID %in% ADAE_FILTERED$USUBJID) {
          chunks_push(bquote({
            ...
            teal::variable_labels(ADAE)[.(ae_line_col_var)] <- teal::get_variable_labels(ADAE_FILTERED, fill = FALSE)[.(ae_line_col_var)]
          }))
          chunks_safe_eval()
