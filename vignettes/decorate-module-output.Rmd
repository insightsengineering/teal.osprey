...
app <- init(
  ...
  modules = modules(
    tm_g_spiderplot(
      paramcd = variables(choices = teal.picks::is_categorical(),
                          selected = "PARAMCD"),
      decorators = list(
...
app <- init(
  ...
  modules = modules(
    tm_g_ae_sub(
      arm_var = variables(choices = is_categorical(),
                          selected = "ACTARMCD"),
      group_var = variables(choices = is_categorical(),
...
Here's an example to showcase how you can edit an output of class `cowplot`.
You can extend them using `grid` and `gridExtra` functions.

```{r decorate_cowplot, message=FALSE, eval=FALSE}
library(teal.osprey)
...
code <- paste0(c(
  "interactive <- function() TRUE",
  knitr::knit_code$get("setup"),
  knitr::knit_code$get("decorate_cowplot")
), collapse = "\n")
